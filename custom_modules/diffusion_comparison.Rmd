...
```{r}
exact<- read.csv("../corrected_exact/voxel_values_1.csv")
#dt_0p01<-seq(0,0.9,0.1)
solution_0p01<- read.csv("../corrected_EG_dt_0p01_noCells_each_second/voxel_values_1.csv")
solution_0p02<- read.csv("../corrected_0p02/voxel_values_1.csv")
solution_0p05<- read.csv("../corrected_0p05/voxel_values_1.csv")
solution_0p1<- read.csv("../corrected_0p1/voxel_values_1.csv")
solution_0p2<- read.csv("../corrected_0p2/voxel_values_1.csv")
solution_0p5<- read.csv("../corrected_0p5/voxel_values_1.csv")
...
```{r}
exact<- exact %>% select(current_time,concentration_EG,voxel_id) %>% arrange(voxel_id,current_time)
solution_0p01<- solution_0p01 %>% select(current_time,concentration_EG,voxel_id) %>% arrange(voxel_id,current_time)
solution_0p02<- solution_0p02 %>% select(current_time,concentration_EG,voxel_id) %>% arrange(voxel_id,current_time)
solution_0p05<- solution_0p05 %>% select(current_time,concentration_EG,voxel_id) %>% arrange(voxel_id,current_time)
solution_0p1<- solution_0p1 %>% select(current_time,concentration_EG,voxel_id) %>% arrange(voxel_id,current_time)
solution_0p2<- solution_0p2 %>% select(current_time,concentration_EG,voxel_id) %>% arrange(voxel_id,current_time)
solution_0p5<- solution_0p5 %>% select(current_time,concentration_EG,voxel_id) %>% arrange(voxel_id,current_time)
#acc_voxels_0p01_total <- acc_voxels_current
#unique(acc_voxels_0p01_1$current_time)
test_difference <- abs(exact$concentration_EG - solution_0p01$concentration_EG)
max(test_difference)
```
...
{
  exact_select <- exact %>% filter(current_time==dt_0p01[j])
