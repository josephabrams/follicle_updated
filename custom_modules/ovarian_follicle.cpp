...
if(counting_time%200==0)
  {
  std::cout<<"VOLUME:"<<pCell->phenotype.volume.total<<"\n"; 
  }	
  std::vector <double> basement_membrane_center={0.0,0.0,0.0};
	 // std::cout<<"number of springs: "<<all_spring_cells.size()<<"\n";
	  for (size_t i=0; i<all_spring_cells.size();i++)
	   {
	  	// std::cout<<"all springs by index: "<< all_spring_cells[i]->m_my_pCell<<"\n";
	   	// std::cout<<"has neighbors "<< spring_cell_by_pCell_index[i]->m_springs.size()<<"\n";
	   	// std::cout<<" IS basement? "<< all_spring_cells[i]->is_basement_connected<<"\n";
	   	// std::cout<<" Volume is "<< all_spring_cells[i]->m_my_pCell->phenotype.volume.total<<"\n";
	...
	// // //  	// }
		 // update_concentrations(all_spring_cells[i]);
	 		//basement_membrane_mechanics(spring_cell_by_pCell_index[i], 100, basement_membrane_center);
	    }
	...
	//two_parameter_single_step(pCell,phenotype,dt);
  // std::cout<<"osmoles per kg"<< osmolal<<"\n";
	return;
...
void oocyte_phenotype_rule( Cell* pCell, Phenotype& phenotype, double dt )
{
  Spring_Cell* SPcell=spring_cell_by_pCell_index[pCell->index];
	...
	// ofs << PhysiCell_globals.current_time<<", "<< pCell->phenotype.volume.total<<", "<<SPcell->exterior_osmolality<<", "<<SPcell->interior_osmolality <<", "<<SPcell->interior_molarity<<", "<<SPcell->exterior_molarity<<PhysiCell_settings.omp_num_threads<<"\n";
	// ofs.close();
 if(counting_time%20)
  {
    // double test_concentration =concentration_at_boundary(pCell,1);
    // std::cout<<"concentration_at_boundary: "<< test_concentration<<"\n";
   ...
   // for (size_t i=0; i<all_spring_cells.size();i++)
	   // {
	  	// std::cout<<"exterior concentrations: "<< all_spring_cells[i]->exterior_molarity<<"\n";
	  	// std::cout<<"exterior moles: "<< all_spring_cells[i]->exterior_component_molality<<"\n";
	  	// std::cout<<"exterior osmoles: "<< all_spring_cells[i]->exterior_osmolality<<"\n";
    // } 
	// std::ofstream ofs;
	// ofs.open ("./output/middle_diffusion_2D.csv", std::ofstream::out | std::ofstream::app);
	// ofs << PhysiCell_globals.current_time<<", "<< test_concentration<<", "<< microenvironment.nearest_density_vector(microenvironment.nearest_voxel_index(pCell->position))<<", "<<PhysiCell_settings.omp_num_threads<<"\n";
	// ofs.close();
  }
  counting_time++;
	return;
...
void granulosa_phenotype_rule( Cell* pCell, Phenotype& phenotype, double dt )
{
 if(counting_time%200==0)
  {
    double test_concentration =concentration_at_boundary(pCell,0);
    // std::cout<<"concentration_at_boundary: "<< test_concentration<<"\n";
    // std::cout<<"Size initial "<< default_microenvironment_options.initial_condition_vector.size()<<"\n"; 
    // std::cout<<"desnity size initial "<< microenvironment.number_of_densities()<<"\n"; 
   for (size_t i=0; i<all_spring_cells.size();i++)
	   {
	  	// std::cout<<"exterior concentrations: "<< all_spring_cells[i]->exterior_molarity<<"\n";
	  	// std::cout<<"exterior moles: "<< all_spring_cells[i]->exterior_component_molality<<"\n";
	  	// std::cout<<"exterior osmoles: "<< all_spring_cells[i]->exterior_osmolality<<", "<<"Position: "<< all_spring_cells[i]->m_my_pCell->position<<"\n";
    } 
	// std::ofstream ofs;
	// ofs.open ("./output/middle_diffusion_2D.csv", std::ofstream::out | std::ofstream::app);
	// ofs << PhysiCell_globals.current_time<<", "<< test_concentration<<", "<< microenvironment.nearest_density_vector(microenvironment.nearest_voxel_index(pCell->position))<<", "<<PhysiCell_settings.omp_num_threads<<"\n";
...
void setup_4_granulosa_test_case(void)
{
	...
	create_spring_cell_from_cell(pCell_granulosa3b, pCell_granulosa3b->phenotype); 
