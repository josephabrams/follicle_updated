...
// 
void two_p(double* Lp, double* R, double* T, double* surface_area, double* exterior_osmolality, double* internal_osmolality, std::vector<double>* dVw, std::vector<double> *exterior_molarity, std::vector<double> *Ps, std::vector <double> *internal_molarity, std::vector<double> *dN, std::vector<double> *previous_dN, double dt, double *water_volume, double *previous_water_volume, std::vector <double> *moles, std::vector <double> *previous_moles )
{
    ...
    if(PhysiCell_globals.current_time<dt)//forward euler
    {
        ...
        for (size_t i = 0; i < (*moles).size(); i++)
        {
            ...
            (*uptake_concentrations)[i]=moles[i]/water_volume[i]
        }
    }
