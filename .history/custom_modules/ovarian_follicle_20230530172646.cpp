...
void oocyte_cell_rule( Cell* pCell, Phenotype& phenotype, double dt )
{
	...
	for (size_t i=0; i<cell_definitions_by_index.size();i++)
	 {
		std::cout<<"DEFINE "<<i<<" of "<<cell_definitions_by_index[i]->phenotype.volume.total<< " for "<<cell_definitions_by_index.size()<<"\n";
