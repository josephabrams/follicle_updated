...
Cryo_State_Variables::Cryo_State_Variables(int solute_num)
{
    ...
    double exterior_osmolality=0.0;//total exterior osmolality salt+CPA (mole/kg)
    double interior_osmolality=0.0;// total internal osmolality salt+CPA
    std::vector<double> internal_molarity.(solute_num,0.0);
    std::vector<double> external_molarity.(solute_num,0.0);
    std::vector<double> Ps.(solute_num,0.0);;
    double Lp;
  return;
}
