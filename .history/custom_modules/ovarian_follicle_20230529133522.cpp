...
void oocyte_cell_rule( Cell* pCell, Phenotype& phenotype, double dt )
{
	...
	{
		std::cout<<"vector size"<< spring_cell_by_pCell_index[i]->m_springs.size()<<"\n";
		for (size_t j = 0; j <spring_cell_by_pCell_index[i]->m_springs.size()-1 ; i++)
		{
			std::cout<<"all springs by index: "<< i<< ", "<< j<< ", "<< spring_cell_by_pCell_index[i]->m_springs[j]
			std::cout<<"all springs by index: "<< i<< ", "<< j<< ", "<< spring_cell_by_pCell_index[i]->m_springs[j]->m_pNeighbor<<"\n";
		}
