...
void dVw_osmolality(double *Lp, double *R, double *T, double *surface_area, double *exterior_osmolality, double *internal_osmolality, double *dVw, double *previous_dVw )
{
...
{
    double current_water_volume=(*water_volume);//to avoid parallelization issues
    ...
    else //adams bashhforth second order
    {
        ...
        for (size_t i = 0; i < (*moles).size(); i++)
        {
            (*internal_molarity)[i]=(*moles)[i]/current_water_volume;
    ...
    {
        (*uptake_concentrations)[i]=(*moles)[i]/(current_water_volume);
    }
