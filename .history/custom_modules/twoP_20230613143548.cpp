...
void Adams_Bashforth_ODE_2nd_Order(std::vector< std::vector<double>> *Y, std::vector<double> *df_dts, std::vector<double> *previous_df_dts, double step_size) {
  // vector form of Y_{n+1}=Y_{n}+h/2(3*(F(x_{n},t_{n})-F(x_{n-1}, t_{n-})))
  for (unsigned int i = 0; i < (*Y)[0].size(); i++) {
    (*Y)[1][i] = (*Y_current)[0][i] + (step_size / 2) * (3 * ((*df_dts)[i] - (*previous_df_dts)[i]));
  }
  return;
}
