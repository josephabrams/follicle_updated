...
void create_oocyte_cell_type(void)
{
	...
	*/
	...
	oocyte_cell.custom_data.add_variable("spring_constant","unitless",0.3);
...
{
	non_connected_neighboor_pressure(pCell,dt);
	std::ofstream ofs;
	ofs.open ("./oocyte_test_file.csv", std::ofstream::out | std::ofstream::app);
...
void granulosa_velocity_rule( Cell* pCell, Phenotype& phenotype, double dt )
{
	...
	if (PhysiCell_globals.current)
	{
		
	}	
	std::ofstream ofs;
	ofs.open ("./test_file.csv", std::ofstream::out | std::ofstream::trunc);
	ofs<<"\n";
	ofs.close();
	ofs.open ("./test_file.csv", std::ofstream::out | std::ofstream::app);
	ofs << PhysiCell_globals.current_time<<", "<< pCell<<", "<< pCell->position[0]<<", "<<pCell->position[1]<< ", "<< pCell->position[2]<<", "<<pCell->velocity <<", "<<omp_get_thread_num()<<"\n";
	ofs.close();
...
*/
	...
	microenvironment.diffusion_coefficients[0] = 530;
	microenvironment.decay_rates[0] = 0;

	microenvironment.add_density( "GLY", "dimensionless" );
	microenvironment.diffusion_coefficients[1] = 530;//530; //TODO: check about the diffusivity of this value
...
void setup_4_granulosa_test_case(void)
{
	...
	pCell_granulosa1a->assign_position(7,28,0 );
	...
	pCell_granulosa2a->assign_position(68,7,0 );
	...
	pCell_granulosa2b->assign_position(68,-7,0 );
	...
	pCell_granulosa3a->assign_position(7,-68,0 );
	...
	pCell_granulosa3b->assign_position(-7,-68,0 );
	...
	pCell_granulosa4a->assign_position(-68,7,0 );
	...
	pCell_granulosa4b->assign_position(-68,-7,0 );
