...
void two_p(double const *Lp, double const *R, double const *T, double *surface_area, double *exterior_osmolality, double *internal_osmolality, std::vector<double> *dVw, double *exterior_molarity, double *internal_molarity, std::vector<double> *dN, double dt, std::vector <double> *water_volume )
{
    double previous_water_volume=(*water_volume)[1];//current
    double previous_N=(*dN)[1];//current
    if(PhysiCell_globals.current_time<dt)//forward euler
    {
        ...
        (*cell_volume)=previous_cell_volume+((*dVw)[1]*dt);
