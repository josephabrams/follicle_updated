...
void oocyte_cell_rule( Cell* pCell, Phenotype& phenotype, double dt )
{
	...
	for (size_t j = 0; j <spring_cell_by_pCell_index[pCell->index+1]->m_springs.size() ; j++)
	{
		std::cout<<spring_cell_by_pCell_index[pCell->index]<<std::endl;
		std::cout<<"connected to : "<< j<< ", "<< spring_cell_by_pCell_index[pCell->index+1]->m_springs[j]->m_pNeighbor<<"\n";
	}
	/*
	for (size_t i=0; i<all_spring_cells.size();i++)
	{
		std::cout<<"all springs by index: "<< spring_cell_by_pCell_index[i]->m_my_pCell<<"\n";
		std::cout<<"has neighbors "<< spring_cell_by_pCell_index[i]->m_springs.size()-1<<"\n";
		for (size_t j = 0; j <spring_cell_by_pCell_index[i]->m_springs.size()-1 ; j++)
		{
			std::cout<<"connected to : "<< i<< ", "<< j<<std::endl;
			std::cout<<"connected to : "<< i<< ", "<< j<< ", "<< spring_cell_by_pCell_index[i]->m_springs[j]->m_pNeighbor<<"\n";
		}
	...
	*/
	...
	// std::ofstream ofs;
	// ofs.open ("./output/oocyte_test_file.csv", std::ofstream::out | std::ofstream::app);
...
{
		for(int i=0; i<all_spring_cells.size();i++)
	{
		//std::cout<<PhysiCell_globals.current_time<<"spring cells"<< all_spring_cells[i]<<"\n";
		//std::cout<<PhysiCell_globals.current_time<<"by index"<<spring_cell_by_pCell_index[i]->m_springs.size()<<"\n";
...
void granulosa_velocity_rule( Cell* pCell, Phenotype& phenotype, double dt )
{
	...
	{
		ofs.open ("./output/test_file2.csv", std::ofstream::out | std::ofstream::app);
		ofs << PhysiCell_globals.current_time<<", "<< pCell<<", "<< pCell->position[0]<<", "<<pCell->position[1]<< ", "<< pCell->position[2]<<", "<<pCell->velocity <<", "<<omp_get_thread_num()<<"\n";
		ofs.close();
	}
	//std::cout<< "Granulosa Velocity: "<< pCell->velocity<<std::endl;
...
void setup_4_granulosa_test_case(void)
{
	...
	pCell_granulosa2b->assign_position(66,-7.8,0 );
	...
	pCell_granulosa4b->assign_position(-66.6,-6,0 );
