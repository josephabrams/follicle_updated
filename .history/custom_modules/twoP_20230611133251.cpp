...
{
    double previous_cell_volume=*cell_volume;
    if(PhysiCell_globals.current_time<dt)//forward euler
    {
        dVw_osmolality(*Lp, double const *R, double const *T, double *surface_area, double *exterior_osmolality, double *internal_osmolality, std::vector<double> *dVw);
        (*cell_volume)[1]=
